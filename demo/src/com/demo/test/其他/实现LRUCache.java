...
public class 实现LRUCache {
    ...
    /**
     * 输入转化, 将各种格式的输入转化为 LRUCache 的 set 或 get 操作
     * <p>
    ...
    public static class LRUCache {
        ...
        // 有序的 hashMap, 用于删除头部较久未使用的元素
        // put时会将key删除, put之后key将存于尾部, 并且判断是否超出size, 超出则移除头部内容, 保障尾部始终是最新的
        // get时也将key删除, get之后key将存于尾部, 保障尾部始终是最新的
        ...
        public void put(int key, int value) {
            ...
            if (map.size() >= size) {
                // map.keySet().iterator().next() 从头部删除(先添加的先删除)
